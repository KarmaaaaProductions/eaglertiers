...
tierlists.forEach(tier => {
    ...
    if (tier.toLowerCase() === 'overall') {  // Since we're on crystal.html
...
function parseTierData(fileContent, gameType) {
    ...
    lines.forEach(line => {
        // Skip header lines that are in brackets
        if (line.startsWith('[') && line.endsWith(']')) {
            return;
        }
        
        // Check if line is a tier marker
        if (line.startsWith('LT') || line.startsWith('HT')) {
            currentTier = `${gameType}-${line}`;
            rankings[currentTier] = [];
        } 
        // If we have a current tier and the line isn't empty, it must be a player name
...
function calculatePoints(tier) {
    const tierLevel = tier.split('-')[1]; // Remove the game type prefix
    const pointsMap = {
        'LT5': 1,
        'HT5': 2,
        'LT4': 3,
...
function displayLeaderboard(playerData) {
    ...
    players.forEach((player, index) => {
        ...
        // Create tier info for each game type
        Object.entries(tiersByGame).forEach(([gameType, tiers]) => {
            ...
            tiers.forEach(tier => {
                ...
                tierText.textContent = `${tier.tier} (${tier.points} pts)`;
